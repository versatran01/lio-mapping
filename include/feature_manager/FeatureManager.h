...
struct PointNormalFeature : public Feature {
 public:
  PointNormalFeature() { feature_name = "PointNormalFeature"; }
  PointNormalFeature(const Eigen::Vector3d &point3d_in,
                     const Eigen::Vector3d &normal3d_in) {
  ...
  PointNormalFeature &GetFeatureInner() { return *this; }

  void GetFeature(Feature *feature) {
    PointNormalFeature *derived_feature =
        static_cast<PointNormalFeature *>(feature);
  ...
  Eigen::Matrix3d covariance;  /// the covariance in the current frame from the
                               /// normal vector
...
struct PointPlaneFeature : public Feature {
  PointPlaneFeature() { feature_name = "PointPlaneFeature"; }

  PointPlaneFeature(const Eigen::Vector3d &point_in,
                    const Eigen::Vector4d &coeffs_in) {
    feature_name = "PointPlaneFeature";
    point = point_in;
    coeffs = coeffs_in;
  }

  PointPlaneFeature &GetFeatureInner() { return *this; }

  void GetFeature(Feature *feature) {
    PointPlaneFeature *derived_feature =
        static_cast<PointPlaneFeature *>(feature);
    *derived_feature = this->GetFeatureInner();
  }
...
class FeatureManager {
 public:
  FeatureManager();
  //  static void CalculateFeatures(const pcl::KdTreeFLANN<PointT>::Ptr
  //  &kdtree_surf_from_map,
  //                                const PointCloudPtr
  //                                &local_surf_points_filtered_ptr, const
  //                                PointCloudPtr &surf_stack, const Transform
  //                                &local_transform, vector<PointPlaneFeature>
  //                                &features);
  //
  //  static void CalculateLaserOdom(const pcl::KdTreeFLANN<PointT>::Ptr
  //  &kdtree_surf_from_map,
  //                                const PointCloudPtr
  //                                &local_surf_points_filtered_ptr, const
  //                                PointCloudPtr &surf_stack, const Transform
  ...
  //                                &kdtree_corner_from_map, const PointCloudPtr
  //                                &local_corner_points_filtered_ptr, const
  //                                PointCloudPtr &corner_stack, const Transform
  //                                &local_transform, vector<PointPlaneFeature>
  //                                &features);
  //
  //  static void CalculateLaserOdom(const pcl::KdTreeFLANN<PointT>::Ptr
  ...
  //                                 PointCloudPtr
  //                                 &local_corner_points_filtered_ptr, const
  //                                 PointCloudPtr &corner_stack, const

...
class ImuGravityFactor : public ceres::SizedCostFunction<15, 7, 9, 7, 9, 4> {
 public:
  ...
  ImuGravityFactor(std::shared_ptr<IntegrationBase> pre_integration)
      : pre_integration_{pre_integration},
        g_norm_{pre_integration->config_.g_norm} {
    GI_ = g_norm_ * GI_dir_;
  }
  virtual bool Evaluate(double const *const *parameters, double *residuals,
    ...
    Eigen::Quaterniond Qwi(parameters[4][3], parameters[4][0], parameters[4][1],
                           parameters[4][2]);
    ...
    residual = pre_integration_->Evaluate(Pi, Qi, Vi, Bai, Bgi, Pj, Qj, Vj, Baj,
                                          Bgj, GW);
    ...
    if (jacobians) {
      ...
      Eigen::Matrix3d dp_dba =
          pre_integration_->jacobian_.template block<3, 3>(O_P, O_BA);
      Eigen::Matrix3d dp_dbg =
          pre_integration_->jacobian_.template block<3, 3>(O_P, O_BG);

      Eigen::Matrix3d dq_dbg =
          pre_integration_->jacobian_.template block<3, 3>(O_R, O_BG);

      Eigen::Matrix3d dv_dba =
          pre_integration_->jacobian_.template block<3, 3>(O_V, O_BA);
      Eigen::Matrix3d dv_dbg =
          pre_integration_->jacobian_.template block<3, 3>(O_V, O_BG);
      ...
      if (jacobians[0]) {
        ...
        jacobian_pose_i.block<3, 3>(O_P, O_R) =
            SkewSymmetric(Qi.inverse() * (-0.5 * GW * sum_dt * sum_dt + Pj -
                                          Pi - Vi * sum_dt));
        ...
        jacobian_pose_i.block<3, 3>(O_R, O_R) =
            -(LeftQuatMatrix(Qj.inverse() * Qi) *
              RightQuatMatrix(corrected_delta_q))
                 .topLeftCorner<3, 3>();
#endif
      ...
      if (jacobians[1]) {
        ...
        jacobian_speedbias_i.block<3, 3>(O_R, O_BG - O_V) =
            -LeftQuatMatrix(Qj.inverse() * Qi * corrected_delta_q)
                 .topLeftCorner<3, 3>() *
            dq_dbg;
#endif
      ...
      if (jacobians[2]) {
        ...
        jacobian_pose_j.block<3, 3>(O_R, O_R) =
            LeftQuatMatrix(corrected_delta_q.inverse() * Qi.inverse() * Qj)
                .topLeftCorner<3, 3>();
#endif
      ...
      if (jacobians[4]) {
        Eigen::Map<Eigen::Matrix<double, 15, 4, Eigen::RowMajor>>
            jacobian_gravity(jacobians[4]);
        jacobian_gravity.setZero();

        Eigen::Matrix<double, 3, 3> dP_dg =
            0.5 * Qi.inverse().toRotationMatrix() * Qwi.toRotationMatrix() *
            SkewSymmetric(GI_) * sum_dt * sum_dt;
        jacobian_gravity.block<3, 2>(O_P, 0) = dP_dg.leftCols<2>();

        Eigen::Matrix<double, 3, 3> dV_dg = Qi.inverse().toRotationMatrix() *
                                            Qwi.toRotationMatrix() *
                                            SkewSymmetric(GI_) * sum_dt;
  ...
  void Check(double const *const *parameters) {
    ...
    for (int k = 0; k < 33; k++) {
      ...
      Bai =
          Eigen::Vector3d(parameters[1][3], parameters[1][4], parameters[1][5]);
      Bgi =
          Eigen::Vector3d(parameters[1][6], parameters[1][7], parameters[1][8]);

      Pj =
          Eigen::Vector3d{parameters[2][0], parameters[2][1], parameters[2][2]};
      Qj = Eigen::Quaterniond(parameters[2][6], parameters[2][3],
                              parameters[2][4], parameters[2][5]);

      Vj =
          Eigen::Vector3d(parameters[3][0], parameters[3][1], parameters[3][2]);
      Baj =
          Eigen::Vector3d(parameters[3][3], parameters[3][4], parameters[3][5]);
      Bgj =
          Eigen::Vector3d(parameters[3][6], parameters[3][7], parameters[3][8]);

      Qwi = Eigen::Quaterniond(parameters[4][3], parameters[4][0],
      ...
      tmp_residual = pre_integration_->Evaluate(Pi, Qi, Vi, Bai, Bgi, Pj, Qj,
                                                Vj, Baj, Bgj, GW);
 ...
 private:
  Eigen::Vector3d GI_dir_{
      0.0, 0.0, -1.0};  ///< direction of gravity in an inertial reference

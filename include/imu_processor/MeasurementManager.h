...
// typedef vector<pair<vector<ImuMsgConstPtr>, OdomMsgConstPtr> >
// PairMeasurements;
...
struct MeasurementManagerConfig {
  ...
  string laser_odom_topic =
      "/aft_mapped_to_init";  // NOTE: Check if the time is too long
  string compact_data_topic =
      "/compact_data";  // NOTE: Check if the time is too long

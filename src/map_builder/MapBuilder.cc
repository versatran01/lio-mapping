...
inline Eigen::Matrix<typename Derived::Scalar, 3, 1> ConstrainedRotAxis(
    const Eigen::QuaternionBase<Derived> &quat_in) {
  ...
  Scalar cot_half_angle =
      (isinf(1 / tan(angle_R / 2)) ? 1e6 : (1 / tan(angle_R / 2)));
  Eigen::Matrix<Scalar, 3, 1> delta_axis_R{-axis_R.y(), axis_R.x(),
                                           cot_half_angle};
...
void MapBuilder::Transform4DAssociateToMap() {
  ...
  Eigen::Vector3d origin_R0 =
      R2ypr(full_transform.rot.normalized().toRotationMatrix().cast<double>());

  Eigen::Vector3d origin_R00 =
      R2ypr(transform_sum_.rot.normalized().toRotationMatrix().cast<double>());
...
void MapBuilder::Transform4DUpdate() {
  //  Eigen::Vector3d origin_R0 =
  //  R2ypr(transform_tobe_mapped_.rot.normalized().toRotationMatrix().cast<double>());
  //  Eigen::Vector3d origin_R00 =
  //  R2ypr(transform_sum_.rot.normalized().toRotationMatrix().cast<double>());
  //
  //  // Z-axix R00 to R0, regard para_pose's R as rotate along the Z-axis first
...
void MapBuilder::SetupRos(ros::NodeHandle &nh) {
  ...
  sub_laser_cloud_corner_last_ = nh.subscribe<sensor_msgs::PointCloud2>(
      "/laser_cloud_corner_last", 2, &PointMapping::LaserCloudCornerLastHandler,
      (PointMapping *)this);
  ...
  sub_laser_full_cloud_ = nh.subscribe<sensor_msgs::PointCloud2>(
      "/full_odom_cloud", 2, &PointMapping::LaserFullCloudHandler,
      (PointMapping *)this);

  sub_laser_odometry_ = nh.subscribe<nav_msgs::Odometry>(
      "/laser_odom_to_init", 2, &PointMapping::LaserOdometryHandler,
      (PointMapping *)this);
...
void MapBuilder::ProcessMap() {
  ...
  //    tf::Quaternion q(tf::Quaternion{q_eigen.x(), q_eigen.y(), q_eigen.z(),
  //    q_eigen.w()}); tf_transform.setRotation(q);
  //
  //
  //    // fetch new input cloud
  //    pcl::PointCloud<pcl::PointXYZI>::Ptr transformed_ptr(new
  //    pcl::PointCloud<pcl::PointXYZI>()); pcl::PointCloud<pcl::PointXYZI>
  //    tmp_cloud;
  //
  //    pcl::transformPointCloud(*laser_cloud_surf_last_, *transformed_ptr,
  //    transform_to_world);
  //
  ...
  //
  //    pcl::transformPointCloud(tmp_cloud, *laser_cloud_surf_last_,
  //    transform_to_world.inverse());
  //  }
  ...
  UpdateMapDatabase(laser_cloud_corner_stack_downsampled_,
                    laser_cloud_surf_stack_downsampled_, laser_cloud_valid_idx_,
                    transform_tobe_mapped_, cube_center);
#if 0
...
void MapBuilder::OptimizeMap() {
  ...
  Eigen::Matrix3f constrained_R =
      Eigen::Quaternionf::FromTwoVectors(Eigen::Vector3f{1.0, 0.0, 0.0},
                                         constrained_axis)
          .toRotationMatrix();
  ...
  Eigen::Matrix3f projection_mat =
      constrained_axis *
      (constrained_axis.transpose() * constrained_axis).inverse() *
      constrained_axis.transpose();
...
//      Eigen::Vector3f J_r = w.transpose() * RotationVectorJacobian(R_SO3, p);
    ...
    //    DLOG(INFO) << "1. angle_axis_out.axis() bef: " <<
    //    angle_axis_out.axis().transpose(); Eigen::Vector3f X_r =
    //    projection_mat * angle_axis_out.axis(); angle_axis_out.axis() =
    //    angle_axis_out.axis() - X_r; angle_axis_out.axis().normalize();
    //    transform_tobe_mapped_.rot = Eigen::Quaternionf(angle_axis_out);
    //
    //    DLOG(INFO) << "2. X_r: " << X_r.transpose();
    //    DLOG(INFO) << "3. angle_axis_out.axis() aft: " <<
    //    angle_axis_out.axis().transpose();

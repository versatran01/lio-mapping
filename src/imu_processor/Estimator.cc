...
// void PointOdometry::TransformToStart(PointCloudPtr &cloud, Twist<float>
// transform_es, float time_factor) {
...
size_t TransformToEnd(PointCloudPtr &cloud, Twist<float> transform_es,
                      float time_factor, bool keep_intensity = false) {
  size_t cloud_size = cloud->points.size();
...
void Estimator::SetupAllEstimatorConfig(
    const EstimatorConfig &config, const MeasurementManagerConfig &mm_config) {
  ...
  down_size_filter_corner_.setLeafSize(config.corner_filter_size,
                                       config.corner_filter_size,
                                       config.corner_filter_size);
  down_size_filter_surf_.setLeafSize(config.surf_filter_size,
                                     config.surf_filter_size,
                                     config.surf_filter_size);
  down_size_filter_map_.setLeafSize(
      config.map_filter_size, config.map_filter_size, config.map_filter_size);
...
void Estimator::ClearState() {
  ...
  tmp_pre_integration_ = std::make_shared<IntegrationBase>(
      IntegrationBase(acc_last_, gyr_last_, Bas_[cir_buf_count_],
                      Bgs_[cir_buf_count_], estimator_config_.pim_config));
...
void Estimator::SetupRos(ros::NodeHandle &nh) {
  ...
  pub_laser_odom_ =
      nh.advertise<nav_msgs::Odometry>("/predict_laser_odom", 100);
  ...
  pub_plane_normal_ =
      nh.advertise<visualization_msgs::MarkerArray>("/debug/plane_normal", 5);

  pub_local_surf_points_ =
      nh.advertise<sensor_msgs::PointCloud2>("/local/surf_points", 2);
  pub_local_corner_points_ =
      nh.advertise<sensor_msgs::PointCloud2>("/local/corner_points", 2);
  pub_local_full_points_ =
      nh.advertise<sensor_msgs::PointCloud2>("/local/full_points", 2);

  pub_map_surf_points_ =
      nh.advertise<sensor_msgs::PointCloud2>("/map/surf_points", 2);
  pub_map_corner_points_ =
      nh.advertise<sensor_msgs::PointCloud2>("/map/corner_points", 2);
  pub_predict_surf_points_ =
      nh.advertise<sensor_msgs::PointCloud2>("/predict/surf_points", 2);
  pub_predict_corner_points_ =
      nh.advertise<sensor_msgs::PointCloud2>("/predict/corner_points", 2);
  pub_predict_full_points_ =
      nh.advertise<sensor_msgs::PointCloud2>("/predict/full_points", 2);
  pub_predict_corrected_full_points_ = nh.advertise<sensor_msgs::PointCloud2>(
      "/predict/corrected_full_points", 2);

  pub_extrinsic_ =
      nh.advertise<geometry_msgs::PoseStamped>("/extrinsic_lb", 10);
}
...
void Estimator::ProcessLaserOdom(const Transform &transform_in,
                                 const std_msgs::Header &header) {
  ...
  if (stage_flag_ != INITED &&
      laser_odom_recv_count_ % estimator_config_.init_window_factor !=
          0) {  /// better for initialization
    return;
  }
  ...
  opt_cube_centers_.push(CubeCenter{laser_cloud_cen_length_,
  ...
  if (stage_flag_ != INITED ||
      (!estimator_config_.enable_deskew && !estimator_config_.cutoff_deskew)) {
    surf_stack_.push(
        boost::make_shared<PointCloud>(*laser_cloud_surf_stack_downsampled_));
        ...
        if (cir_buf_count_ == estimator_config_.window_size) {
          ...
          if (!estimator_config_.imu_factor) {
            ...
            if (extrinsic_stage_ == 2) {
              ...
              bool extrinsic_result = ImuInitializer::EstimateExtrinsicRotation(
                  all_laser_transforms_, transform_lb_);
              LOG(INFO) << ">>>>>>> extrinsic calibration"
                        << (extrinsic_result ? " successful" : " failed")
                        << " <<<<<<<";
              if (extrinsic_result) {
                extrinsic_stage_ = 1;
            ...
            if (extrinsic_stage_ != 2 &&
                (header.stamp.toSec() - initial_time_) > 0.1) {
          ...
          if (init_result) {
            ...
            Q_WI_ = R_WI_;
            //            wi_trans_.setRotation(tf::Quaternion{Q_WI_.x(),
            //            Q_WI_.y(), Q_WI_.z(), Q_WI_.w()});

            ROS_WARN_STREAM(">>>>>>> IMU initialized <<<<<<<");

            if (estimator_config_.enable_deskew ||
                estimator_config_.cutoff_deskew) {
              ros::ServiceClient client =
                  nh_.serviceClient<std_srvs::SetBool>("/enable_odom");
              ...
              if (client.call(srv)) {
                ...
                LOG(FATAL)
                    << "FAILED TO CALL TURNING OFF THE ORIGINAL LASER ODOM";
              }
            }
          ...
          DLOG(INFO) << "all_laser_transforms_ size: "
                     << all_laser_transforms_.size();
      ...
      case INITED: {
        ...
        if (opt_point_coeff_map_.size() ==
          ...
          if (estimator_config_.enable_deskew ||
            ...
            //            vel1 = Rs_[estimator_config_.window_size -
            //            1].transpose() * Vs_[estimator_config_.window_size -
            //            1]; vel2 =
            //            Rs_[estimator_config_.window_size].transpose() *
            //            Vs_[estimator_config_.window_size]; body_velocity =
            //            (vel1 + vel2) / 2;

            Transform transform_body_es = transform_e.inverse() * transform_s;
            //            transform_body_es.pos = -0.1 *
            //            body_velocity.cast<float>();
            {
            ...
            transform_es_ =
                transform_lb_ * transform_body_es * transform_lb_.inverse();
            ...
            size_t laser_cloud_surf_stack_ds_size =
                laser_cloud_surf_stack_downsampled_->points.size();
            ...
            down_size_filter_corner_.filter(
                *laser_cloud_corner_stack_downsampled_);
            size_t laser_cloud_corner_stack_ds_size =
                laser_cloud_corner_stack_downsampled_->points.size();

            surf_stack_.push(boost::make_shared<PointCloud>(
                *laser_cloud_surf_stack_downsampled_));
            size_surf_stack_.push(laser_cloud_surf_stack_downsampled_->size());

            corner_stack_.push(boost::make_shared<PointCloud>(
                *laser_cloud_corner_stack_downsampled_));
            size_corner_stack_.push(
          ...
          if (!opt_point_coeff_mask_.first()) {
            UpdateMapDatabase(opt_corner_stack_.first(),
                              opt_surf_stack_.first(), opt_valid_idx_.first(),
                              opt_transforms_.first(),
                              opt_cube_centers_.first());

            DLOG(INFO)
                << "all_laser_transforms_: "
                << all_laser_transforms_[estimator_config_.window_size -
                                         estimator_config_.opt_window_size]
                       .second.transform;
            DLOG(INFO) << "opt_transforms_: " << opt_transforms_.first();
          }

        } else {
          LOG(ERROR) << "opt_point_coeff_map_.size(): "
                     << opt_point_coeff_map_.size()
                     << " != estimator_config_.opt_window_size + 1: "
                     << estimator_config_.opt_window_size + 1;
        }
  ...
  wi_trans_.setRotation(
      tf::Quaternion{Q_WI_.x(), Q_WI_.y(), Q_WI_.z(), Q_WI_.w()});
  wi_trans_.stamp_ = header.stamp;
  tf_broadcaster_est_.sendTransform(wi_trans_);
}

void Estimator::ProcessCompactData(
    const sensor_msgs::PointCloud2ConstPtr &compact_data,
    const std_msgs::Header &header) {
  /// 1. process compact data
  PointMapping::CompactDataHandler(compact_data);

  if (stage_flag_ == INITED) {
    Transform trans_prev(
        Eigen::Quaterniond(Rs_[estimator_config_.window_size - 1])
            .cast<float>(),
    ...
    Transform transform_incre(transform_bef_mapped_.inverse() *
                              transform_sum_.transform());

    //    DLOG(INFO) << "base incre in laser world: " << d_trans;
    //    DLOG(INFO) << "incre in laser world: " << transform_incre;
    //    DLOG(INFO) << "before opt: " << transform_tobe_mapped_ * transform_lb_
    //    * d_trans * transform_lb_.inverse();
    //
    ...
    if (estimator_config_.imu_factor) {
      ...
      transform_tobe_mapped_bef_ = transform_tobe_mapped_ * transform_lb_ *
                                   d_trans * transform_lb_.inverse();
      transform_tobe_mapped_ = transform_tobe_mapped_bef_;
    } else {
      TransformAssociateToMap();
      DLOG(INFO) << ">>>>> transform original tobe <<<<<: "
  ...
  DLOG(INFO) << endl
             << "transform_aft_mapped_[" << header.stamp.toSec()
             << "]: " << transform_aft_mapped_;
...
bool Estimator::RunInitialization() {
  ...
  for (size_t i = 0; i < estimator_config_.window_size + 1; ++i) {
    ...
    Ps_[i] = trans_bi.pos.template cast<double>();
    Rs_[i] =
        trans_bi.rot.normalized().toRotationMatrix().template cast<double>();
...
#ifdef USE_CORNER
void Estimator::CalculateFeatures(
...
void Estimator::CalculateFeatures(
  ...
  for (int i = 0; i < surf_points_size; i++) {
    ...
    if (point_search_sq_dis[num_neighbors - 1] < min_match_sq_dis_) {
      ...
      for (int j = 0; j < num_neighbors; j++) {
        if (fabs(
                pa * local_surf_points_filtered_ptr->points[point_search_idx[j]]
                         .x +
                pb * local_surf_points_filtered_ptr->points[point_search_idx[j]]
                         .y +
                pc * local_surf_points_filtered_ptr->points[point_search_idx[j]]
                         .z +
                pd) > min_plane_dis_) {
          planeValid = false;
          break;
      ...
      if (planeValid) {
        ...
        PointAssociateToMap(point_on_z_axis, point_on_z_axis,
                            transform_to_local);
        ...
        if (s > 0.1 && is_in_laser_fov) {
          unique_ptr<PointPlaneFeature> feature =
              std::make_unique<PointPlaneFeature>();
          feature->score = s;
          feature->point =
              Eigen::Vector3d{point_ori.x, point_ori.y, point_ori.z};
          feature->coeffs =
              Eigen::Vector4d{coeff1.x, coeff1.y, coeff1.z, coeff1.intensity};
    ...
    if (point_search_sq_dis[4] < min_match_sq_dis_) {
      ...
      if (mat_D1(0, 2) > 3 * mat_D1(0, 1)) {
        ...
        Eigen::Vector3f normal_to_point =
            ((X1 - X2).cross(a012_vec)).normalized();
        ...
        if (s > 0.1 && is_in_laser_fov) {
          unique_ptr<PointPlaneFeature> feature1 =
              std::make_unique<PointPlaneFeature>();
          ...
          unique_ptr<PointPlaneFeature> feature2 =
              std::make_unique<PointPlaneFeature>();
          feature2->score = s * 0.5;
          feature2->point =
              Eigen::Vector3d{point_ori.x, point_ori.y, point_ori.z};
          feature2->coeffs =
              Eigen::Vector4d{coeff2.x, coeff2.y, coeff2.z, coeff2.intensity} *
              0.5;
...
#ifdef USE_CORNER
void Estimator::CalculateLaserOdom(
    const pcl::KdTreeFLANN<PointT>::Ptr &kdtree_surf_from_map,
    const PointCloudPtr &local_surf_points_filtered_ptr,
    const PointCloudPtr &surf_stack,
    const pcl::KdTreeFLANN<PointT>::Ptr &kdtree_corner_from_map,
...
void Estimator::CalculateLaserOdom(
  ...
  for (size_t iter_count = 0; iter_count < num_max_iterations_; ++iter_count) {
    ...
    for (int i = 0; i < laser_cloud_sel_size; i++) {
      ...
      //      Eigen::Vector3f J_r = w.transpose() *
      //      RotationVectorJacobian(R_SO3, p);
      Eigen::Vector3f J_r =
          -w.transpose() * (local_transform.rot * SkewSymmetric(p));
      Eigen::Vector3f J_t = w.transpose();

      float d2 =
          w.transpose() * (local_transform.rot * p + local_transform.pos) +
          coeff.intensity;
...
void Estimator::BuildLocalMap(vector<FeaturePerFrame> &feature_frames) {
  ...
  local_surf_points_filtered_ptr_ =
      boost::make_shared<PointCloud>(PointCloud());
...

  {
    ...
    Quaterniond rot_linearized_pivot(Rs_linearized_pivot *
                                     transform_lb.rot.inverse());
    Eigen::Vector3d pos_linearized_pivot =
        Ps_linearized_pivot - rot_linearized_pivot * transform_lb.pos;

    Twist<double> transform_linearized_pivot =
        Twist<double>(rot_linearized_pivot, pos_linearized_pivot);
#endif
    // endregion
    ...
    if (!init_local_map_) {
      ...
      for (int i = 0; i <= pivot_idx; ++i) {
        ...
        Eigen::Affine3f transform_pivot_i =
            (transform_pivot.inverse() * transform_li)
                .cast<float>()
                .transform();
        pcl::transformPointCloud(*(surf_stack_[i]), transformed_cloud_surf,
                                 transform_pivot_i);
        tmp_cloud_surf += transformed_cloud_surf;

#ifdef USE_CORNER
        pcl::transformPointCloud(*(corner_stack_[i]), transformed_cloud_corner,
                                 transform_pivot_i);
    ...
    for (int i = 0; i < estimator_config_.window_size + 1; ++i) {
      ...
      Twist<double> transform_li = Twist<double>(rot_li, pos_li);
      Eigen::Affine3f transform_pivot_i =
          (transform_pivot.inverse() * transform_li).cast<float>().transform();
      ...
      Quaterniond rot_linearized_i(Rs_linearized_i *
                                   transform_lb.rot.inverse());
      Eigen::Vector3d pos_linearized_i =
          Ps_linearized_i - rot_linearized_i * transform_lb.pos;

      Twist<double> transform_linearized_i =
          Twist<double>(rot_linearized_i, pos_linearized_i);
      Eigen::Affine3f transform_linearized_pivot_i =
          (transform_linearized_pivot.inverse() * transform_linearized_i)
              .cast<float>()
              .transform();
...
//          *local_surf_points_ptr_ = transformed_cloud_surf;
    ...
    DLOG(INFO) << "local_surf_points_ptr_->size() bef: "
               << local_surf_points_ptr_->size();
    down_size_filter_surf_.setInputCloud(local_surf_points_ptr_);
    down_size_filter_surf_.filter(*local_surf_points_filtered_ptr_);
    DLOG(INFO) << "local_surf_points_ptr_->size() aft: "
               << local_surf_points_filtered_ptr_->size();

#ifdef USE_CORNER
    DLOG(INFO) << "local_corner_points_ptr_->size() bef: "
               << local_corner_points_ptr_->size();
    ...
    DLOG(INFO) << "local_corner_points_ptr_->size() aft: "
               << local_corner_points_filtered_ptr_->size();
#endif
  }
  ...
  if (estimator_config_.pcl_viewer) {
    if (normal_vis.init) {
      pcl::PointCloud<pcl::PointXYZ>::Ptr point_world_i_xyz(
  ...
  pcl::KdTreeFLANN<PointT>::Ptr kdtree_surf_from_map(
      new pcl::KdTreeFLANN<PointT>());
  kdtree_surf_from_map->setInputCloud(local_surf_points_filtered_ptr_);

#ifdef USE_CORNER
  pcl::KdTreeFLANN<PointT>::Ptr kdtree_corner_from_map(
      new pcl::KdTreeFLANN<PointT>());
  ...
  for (int idx = 0; idx < estimator_config_.window_size + 1; ++idx) {
    ...
    if (idx > pivot_idx) {
      if (idx != estimator_config_.window_size ||
          !estimator_config_.imu_factor) {
#ifdef USE_CORNER
        CalculateFeatures(kdtree_surf_from_map, local_surf_points_filtered_ptr_,
                          surf_stack_[idx], kdtree_corner_from_map,
                          local_corner_points_filtered_ptr_, corner_stack_[idx],
...
#ifdef USE_CORNER
        CalculateLaserOdom(kdtree_surf_from_map,
                           local_surf_points_filtered_ptr_, surf_stack_[idx],
                           kdtree_corner_from_map,
                           local_corner_points_filtered_ptr_,
                           corner_stack_[idx], local_transforms[idx], features);
#else
        CalculateLaserOdom(kdtree_surf_from_map,
                           local_surf_points_filtered_ptr_, surf_stack_[idx],
                           local_transforms[idx], features);
        ...
        //      int pivot_idx = int(estimator_config_.window_size -
        //      estimator_config_.opt_window_size);
        //
        //      Twist<double> transform_lb = transform_lb_.cast<double>();
        //
        ...
        //
        //      Quaterniond rot_pivot(Rs_pivot * transform_lb.rot.inverse());
        //      Eigen::Vector3d pos_pivot = Ps_pivot - rot_pivot *
        //      transform_lb.pos;
        //
        //      Twist<double> transform_pivot = Twist<double>(rot_pivot,
        //      pos_pivot);
        //
        //      Twist<double> transform_bi = transform_pivot *
        //      local_transforms[idx].cast<double>() * transform_lb; Rs_[idx] =
        //      transform_bi.rot.normalized(); Ps_[idx] = transform_bi.pos;
      }
    } else {
      // NOTE: empty features
    }
    ...
    std::vector<Eigen::Vector4d, Eigen::aligned_allocator<Eigen::Vector4d>>
        plane_coeffs;
    if (estimator_config_.pcl_viewer) {
      pcl::PointCloud<pcl::PointXYZ>::Ptr tmp_cloud_sel(
          new pcl::PointCloud<pcl::PointXYZ>);
      pcl::PointCloud<pcl::Normal>::Ptr tmp_normals_sel(
      ...
      for (int i = 0; i < features.size(); ++i) {
        ...
        tmp_normals_sel->push_back(pcl::Normal{
            float(f.coeffs.x()), float(f.coeffs.y()), float(f.coeffs.z())});
        ...
        //        DLOG(INFO) << p_sel.x * f.coeffs.x() + p_sel.y * f.coeffs.y()
        //        + p_sel.z * f.coeffs.z() + f.coeffs.w();
      }

      if (normal_vis.init) {
        //        pcl::PointCloud<pcl::PointXYZ>::Ptr point_world_i_xyz(new
        //        pcl::PointCloud<pcl::PointXYZ>);
        //        pcl::copyPointCloud(p_world_i, *point_world_i_xyz);
        normal_vis.UpdateCloudAndNormals(tmp_cloud_sel, tmp_normals_sel, 10,
                                         "cloud1", "normal1");
    ...
    feature_per_frame.features.assign(make_move_iterator(features.begin()),
                                      make_move_iterator(features.end()));
...
void Estimator::SolveOptimization() {
  ...
  if (estimator_config_.update_laser_imu) {
    ...
    if (!estimator_config_.imu_factor) {
      Twist<double> incre =
          (transform_lb_.inverse() *
           all_laser_transforms_[cir_buf_count_ - 1]
               .second.transform.inverse() *
           all_laser_transforms_[cir_buf_count_].second.transform *
           transform_lb_)
              .cast<double>();
      Ps_[cir_buf_count_] =
          Rs_[cir_buf_count_ - 1] * incre.pos + Ps_[cir_buf_count_ - 1];
      Rs_[cir_buf_count_] = Rs_[cir_buf_count_ - 1] * incre.rot;
    }

    Twist<double> transform_lb = transform_lb_.cast<double>();
    int pivot_idx =
        int(estimator_config_.window_size - estimator_config_.opt_window_size);
    ...
    for (int i = 0; i < estimator_config_.opt_window_size + 1; ++i) {
      ...
      DLOG(INFO) << "Rs_[" << opt_i << "]: " <<
      Eigen::Quaterniond(Rs_[opt_i]).coeffs().transpose(); DLOG(INFO) << "Bas_["
      << opt_i << "]: " << Bas_[opt_i].transpose(); DLOG(INFO) << "Bgs_[" <<
      opt_i << "]: " << Bgs_[opt_i].transpose();
      */
      // DLOG(INFO) << "transform_lb_: " << transform_lb_;
      // DLOG(INFO) << "gravity in world: " << g_vec_.transpose();

      Twist<double> transform_bi =
          Twist<double>(Eigen::Quaterniond(Rs_[opt_i]), Ps_[opt_i]);
    ...
    //      M15 sqrt_info =
    //          Eigen::LLT<M15>(pre_integrations_[i +
    //          1]->covariance_.inverse()).matrixL().transpose();
    //
    //      V15 res = (pre_integrations_[i + 1]->Evaluate(
    //          Ps_[i], Eigen::Quaterniond(Rs_[i]), Vs_[i], Bas_[i], Bgs_[i +
    //          1], Ps_[i + 1], Eigen::Quaterniond(Rs_[i + 1]), Vs_[i + 1],
    //          Bas_[i + 1], Bgs_[i + 1]));
    //      // DLOG(INFO) << "sqrt_info: " << endl << sqrt_info;
    //
    //      DLOG(INFO) << "imu res bef: " << res.transpose();
    //      // DLOG(INFO) << "weighted pre: " << (sqrt_info * res).transpose();
    //      // DLOG(INFO) << "weighted pre: " << (sqrt_info *
    //      res).squaredNorm();
    //    }
    // endregion
  ...
  if (estimator_config_.marginalization_factor) {
    if (last_marginalization_info) {
      ...
      res_id_marg = problem.AddResidualBlock(
  ...
  if (estimator_config_.imu_factor) {
    for (int i = 0; i < estimator_config_.opt_window_size; ++i) {
      ...
      ceres::internal::ResidualBlock *res_id =
          problem.AddResidualBlock(f, NULL, para_pose_[i], para_speed_bias_[i],
  ...
  if (estimator_config_.prior_factor) {
    {
      ...
      problem.AddResidualBlock(f, NULL, para_ex_pose_);
      //    {
  ...
  if (estimator_config_.marginalization_factor && !turn_off) {
    ...
    //    {
    //      MarginalizationInfo *marginalization_info0 = new
    //      MarginalizationInfo(); if (last_marginalization_info) {
    //        vector<int> drop_set;
    //        for (int i = 0; i <
    ...
    //
    //      if (estimator_config_.imu_factor) {
    //        int pivot_idx = estimator_config_.window_size -
    //        estimator_config_.opt_window_size; if (pre_integrations_[pivot_idx
    //        + 1]->sum_dt_ < 10.0) {
    //          ImuFactor *imu_factor = new
    //          ImuFactor(pre_integrations_[pivot_idx + 1]); ResidualBlockInfo
    //          *residual_block_info = new ResidualBlockInfo(imu_factor, NULL,
    ...
    //            const Eigen::Vector4d &coeff_eigen = feature_j.coeffs;
    //
    //            PivotPointPlaneFactor *pivot_point_plane_factor = new
    //            PivotPointPlaneFactor(p_eigen,
    //                                                                                        coeff_eigen);
    //
    //            ResidualBlockInfo *residual_block_info = new
    //            ResidualBlockInfo(pivot_point_plane_factor, loss_function,
    //                                                                           vector<double *>{para_pose_[0],
    //                                                                                            para_pose_[i],
    //                                                                                            para_ex_pose_},
    //                                                                           vector<int>{0});
    ...
    //        for (int i = 1; i < estimator_config_.opt_window_size + 1; ++i) {
    //          addr_shift2[reinterpret_cast<long>(para_pose_[i])] =
    //          para_pose_[i];
    //          addr_shift2[reinterpret_cast<long>(para_speed_bias_[i])] =
    //          para_speed_bias_[i];
    //        }
    //        addr_shift2[reinterpret_cast<long>(para_ex_pose_)] =
    //        para_ex_pose_;
    //
    //        vector<double *> parameter_blocks2 =
    //        marginalization_info0->GetParameterBlocks(addr_shift2);
    //
    //        vector<ceres::internal::ResidualBlock *> res_ids_marg2;
    //        ceres::internal::ResidualBlock *res_id_marg2 = NULL;
    ////      ceres::Problem problem2;
    //        MarginalizationFactor *marginalization_factor = new
    //        MarginalizationFactor(marginalization_info0); res_id_marg2 =
    ...
    if (estimator_config_.point_distance_factor) {
      for (int i = 1; i < estimator_config_.opt_window_size + 1; ++i) {
        ...
        for (int j = 0; j < features.size(); ++j) {
          ...
          PivotPointPlaneFactor *pivot_point_plane_factor =
              new PivotPointPlaneFactor(p_eigen, coeff_eigen);
    ...
    for (int i = 1; i < estimator_config_.opt_window_size + 1; ++i) {
      ...
      addr_shift[reinterpret_cast<long>(para_speed_bias_[i])] =
    ...
    DLOG(INFO) << "whole marginalization costs: "
               << t_whole_marginalization.Toc();
    ROS_DEBUG_STREAM("whole marginalization costs: "
    ...
    //
    //      vector<double *> parameter_blocks2 =
    //      marginalization_info->GetParameterBlocks(addr_shift2);
    //
    //      vector<ceres::internal::ResidualBlock *> res_ids_marg2;
    //      ceres::internal::ResidualBlock *res_id_marg2 = NULL;
    ////      ceres::Problem problem2;
    //      MarginalizationFactor *marginalization_factor = new
    //      MarginalizationFactor(last_marginalization_info); res_id_marg2 =
    //      problem.AddResidualBlock(marginalization_factor, NULL,
    //                                              parameter_blocks2);
    //      res_ids_marg2.push_back(res_id_marg2);
    //
  ...
  if (estimator_config_.update_laser_imu) {
    ...
    int opt_0 = int(estimator_config_.window_size -
                    estimator_config_.opt_window_size + 0);
    ...
    opt_l0_transform =
        Twist<double>{rot_l0, pos_l0}.cast<float>();  // for updating the map
    ...
    for (int i = 0; i < estimator_config_.opt_window_size + 1; ++i) {
      ...
      Quaterniond rot_li(Rs_[opt_i] *
                         transform_lb.rot.conjugate().normalized());
      Eigen::Vector3d pos_li = Ps_[opt_i] - rot_li * transform_lb.pos;
      Twist<double> transform_li = Twist<double>(rot_li, pos_li);
    ...
    ROS_DEBUG_STREAM("lb in world: "
                     << (rot_l0.normalized() * transform_lb.pos).transpose());
...

    {
      ...
      PublishCloudMsg(pub_map_corner_points_,
                      *local_corner_points_filtered_ptr_, Headers_.last().stamp,
                      "/laser_local");
#endif

      laser_local_trans_.setOrigin(
          tf::Vector3{pos_pivot.x(), pos_pivot.y(), pos_pivot.z()});
      laser_local_trans_.setRotation(tf::Quaternion{
          rot_pivot.x(), rot_pivot.y(), rot_pivot.z(), rot_pivot.w()});
      ...
      PublishCloudMsg(pub_predict_surf_points_, *(surf_stack_.last()),
                      Headers_.last().stamp, "/laser_predict");
      PublishCloudMsg(pub_predict_full_points_, *(full_stack_.last()),
                      Headers_.last().stamp, "/laser_predict");

      {
        // NOTE: full stack into end of the scan
        //        PointCloudPtr tmp_points_ptr =
        //        boost::make_shared<PointCloud>(PointCloud()); *tmp_points_ptr
        //        = *(full_stack_.last()); TransformToEnd(tmp_points_ptr,
        //        transform_es_, 10);
        ...
        PublishCloudMsg(pub_predict_corrected_full_points_,
                        *(full_stack_.last()), Headers_.last().stamp,
                        "/laser_predict");
      }

#ifdef USE_CORNER
      PublishCloudMsg(pub_predict_corner_points_, *(corner_stack_.last()),
                      Headers_.last().stamp, "/laser_predict");
#endif
      laser_predict_trans_.setOrigin(
          tf::Vector3{pos_predict.x(), pos_predict.y(), pos_predict.z()});
      laser_predict_trans_.setRotation(tf::Quaternion{
          rot_predict.x(), rot_predict.y(), rot_predict.z(), rot_predict.w()});
...
void Estimator::VectorToDouble() {
  int i, opt_i,
      pivot_idx = int(estimator_config_.window_size -
                      estimator_config_.opt_window_size);
  P_pivot_ = Ps_[pivot_idx];
  R_pivot_ = Rs_[pivot_idx];
  for (i = 0, opt_i = pivot_idx; i < estimator_config_.opt_window_size + 1;
       ++i, ++opt_i) {
...
void Estimator::DoubleToVector() {
  ...
  // WARNING: not just yaw angle rot_diff; if it is compared with global
  // features, there should be no need for rot_diff
  ...
  Vector3d origin_R00 = R2ypr(Quaterniond(para_pose_[0][6], para_pose_[0][3],
  ...
  if (abs(abs(origin_R0.y()) - 90) < 1.0 ||
    ...
    rot_diff = Rs_[pivot_idx] * Quaterniond(para_pose_[0][6], para_pose_[0][3],
                                            para_pose_[0][4], para_pose_[0][5])
                                    .normalized()
                                    .toRotationMatrix()
                                    .transpose();
...

  {
    ...
    Matrix3d R_opt_pivot =
        rot_diff * Quaterniond(para_pose_[0][6], para_pose_[0][3],
                               para_pose_[0][4], para_pose_[0][5])
                       .normalized()
                       .toRotationMatrix();
    ...
    for (int idx = 0; idx < pivot_idx; ++idx) {
      ...
      Twist<double> trans_opt_idx =
          trans_opt_pivot * trans_pivot.inverse() * trans_idx;
      ...
      //      Rs_[idx] = rot_diff *
      //      trans_pivot_idx.rot.normalized().toRotationMatrix();
    }
  }
  ...
  for (i = 0, opt_i = pivot_idx; i < estimator_config_.opt_window_size + 1;
       ++i, ++opt_i) {
    //    DLOG(INFO) << "para aft: " << Vector3d(para_pose_[i][0],
    //    para_pose_[i][1], para_pose_[i][2]).transpose();

    Rs_[opt_i] = rot_diff * Quaterniond(para_pose_[i][6], para_pose_[i][3],
    ...
    Vs_[opt_i] =
        rot_diff * Vector3d(para_speed_bias_[i][0], para_speed_bias_[i][1],
                            para_speed_bias_[i][2]);

    Bas_[opt_i] = Vector3d(para_speed_bias_[i][3], para_speed_bias_[i][4],
                           para_speed_bias_[i][5]);

    Bgs_[opt_i] = Vector3d(para_speed_bias_[i][6], para_speed_bias_[i][7],
                           para_speed_bias_[i][8]);
  }
  {
    transform_lb_.pos =
        Vector3d(para_ex_pose_[0], para_ex_pose_[1], para_ex_pose_[2])
            .template cast<float>();
    transform_lb_.rot = Quaterniond(para_ex_pose_[6], para_ex_pose_[3],
                                    para_ex_pose_[4], para_ex_pose_[5])
                            .template cast<float>();
  }
}

void Estimator::SlideWindow() {  // NOTE: this function is only for the states
                                 // and the local map

  {
    if (init_local_map_) {
      ...
      Twist<double> transform_li = Twist<double>(rot_li, pos_li);
      Eigen::Affine3f transform_i_pivot =
          (transform_li.inverse() * transform_pivot).cast<float>().transform();
      pcl::ExtractIndices<PointT> extract;

      pcl::transformPointCloud(*(surf_stack_[pivot_idx]),
                               *transformed_cloud_surf_ptr, transform_i_pivot);
...
#ifdef USE_CORNER
      pcl::transformPointCloud(*(corner_stack_[pivot_idx]),
                               *transformed_cloud_corner_ptr,
                               transform_i_pivot);
...
void Estimator::ProcessEstimation() {
  while (true) {
    ...
    //    DLOG(INFO) << "measurement obtained: " << measurements.size() << ",
    //    first imu data size: "
    ...
    for (auto &measurement : measurements) {
      ...
      for (auto &imu_msg : measurement.first) {
        ...
        if (imu_time <= laser_odom_time) {
          ...
          ProcessImu(dt, Vector3d(ax, ay, az), Vector3d(rx, ry, rz),
                     imu_msg->header);
          ...
          ProcessImu(dt_1, Vector3d(ax, ay, az), Vector3d(rx, ry, rz),
                     imu_msg->header);
        }
      }

      DLOG(INFO) << "per imu time: "
                 << tic_toc_imu.Toc() / measurement.first.size() << " ms";
      ...
      //      const auto &quat_from_msg =
      //      compact_data_msg->pose.pose.orientation;

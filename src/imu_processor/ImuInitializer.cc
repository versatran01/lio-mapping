...
void EstimateGyroBias(
  ...
  for (size_t i = 0; i < window_size; ++i) {
    ...
    VectorXd tmp_b(3);
    tmp_b.setZero();
    Eigen::Quaterniond q_ij(laser_trans_i.second.transform.rot.conjugate() *
                            laser_trans_j.second.transform.rot);
    tmp_A =
        laser_trans_j.second.pre_integration->jacobian_.template block<3, 3>(
            O_R, O_BG);  /// Jacobian of dr12_bg
    tmp_b =
        2 * (laser_trans_j.second.pre_integration->delta_q_.conjugate() * q_ij)
                .vec();  /// 2*vec(IMU_ij^T * q_ij)
    A += tmp_A.transpose() * tmp_A;
    b += tmp_A.transpose() * tmp_b;
  }
  delta_bg = A.ldlt().solve(b);
  DLOG(WARNING) << "gyroscope bias initial calibration: "
                << delta_bg.transpose();
...
bool ApproximateGravity(
    CircularBuffer<PairTimeLaserTransform> &all_laser_transforms, Vector3d &g,
  ...
  for (size_t i = 0; i < window_size - 1; ++i) {
    ...
    Vector3d dp12 =
        laser_trans_j.second.pre_integration->delta_p_.template cast<double>();
    Vector3d dp23 =
        laser_trans_k.second.pre_integration->delta_p_.template cast<double>();
    Vector3d dv12 =
        laser_trans_j.second.pre_integration->delta_v_.template cast<double>();
    ...
    Matrix3d rl1 = laser_trans_i.second.transform.rot.template cast<double>()
                       .toRotationMatrix();
    Matrix3d rl2 = laser_trans_j.second.transform.rot.template cast<double>()
                       .toRotationMatrix();
    Matrix3d rl3 = laser_trans_k.second.transform.rot.template cast<double>()
                       .toRotationMatrix();
    ...
    tmp_b = (pl2 - pl1) * dt23 - (pl3 - pl2) * dt12 + (rl2 - rl1) * plb * dt23 -
            (rl3 - rl2) * plb * dt12 + rl2 * rlb * dp23 * dt12 +
            rl1 * rlb * dv12 * dt12 * dt23 - rl1 * rlb * dp12 * dt23;
  ...
  //  g_approx = A.jacobiSvd(Eigen::ComputeThinU |
...
void RefineGravityAccBias(
  ...
  for (int k = 0; k < 5; ++k) {
    ...
    for (size_t i = 0; i < size_velocities - 1; ++i) {
      ...
      Matrix3d rl1 = laser_trans_i.second.transform.rot.normalized()
                         .template cast<double>()
                         .toRotationMatrix();
      Matrix3d rl2 = laser_trans_j.second.transform.rot.normalized()
                         .template cast<double>()
                         .toRotationMatrix();
      Matrix3d rlb = transform_lb.rot.normalized()
                         .template cast<double>()
                         .toRotationMatrix();

      tmp_A.block<3, 3>(0, 0) = dt12 * Matrix3d::Identity();
      //      tmp_A.block<3, 2>(0, 6) = (-0.5 * r_WI * SO3::hat(gI_n) * g_norm *
      //      dt12 * dt12).topLeftCorner<3, 2>(); tmp_b.block<3, 1>(0, 0) =
      //          pl2 - pl1 - rl1 * rlb * dp12 - (rl1 - rl2) * plb - 0.5 * r_WI
      //          * gI_n * g_norm * dt12 * dt12;
      tmp_A.block<3, 2>(0, 6) = 0.5 * Matrix3d::Identity() * lxly * dt12 * dt12;
      tmp_b.block<3, 1>(0, 0) = pl2 - pl1 - rl1 * rlb * dp12 -
                                (rl1 - rl2) * plb -
                                0.5 * g_refined * dt12 * dt12;

      tmp_A.block<3, 3>(3, 0) = Matrix3d::Identity();
      tmp_A.block<3, 3>(3, 3) = -Matrix3d::Identity();
      //      tmp_A.block<3, 2>(3, 6) = (-r_WI * SO3::hat(gI_n) * g_norm *
      //      dt12).topLeftCorner<3, 2>(); tmp_b.block<3, 1>(3, 0) = -rl1 * rlb
      //      * dv12 - r_WI * gI_n * g_norm * dt12;
    ...
    //    R_WI = r_WI * SO3::exp(Vector3d(dg.x(), dg.y(),
    //    0.0)).unit_quaternion();
    //    // WARNING
    //    r_WI = R_WI;
...
// void UpdateVelocities(CircularBuffer<PairTimeLaserTransform>
// &all_laser_transforms,
...
//
//    rl1 = laser_trans_i.second.transform.rot.normalized().template
//    cast<double>().toRotationMatrix(); rl2 =
//    laser_trans_j.second.transform.rot.normalized().template
//    cast<double>().toRotationMatrix(); rlb =
//    transform_lb.rot.normalized().template cast<double>().toRotationMatrix();
//
//    Vs[i] = (pl2 - pl1 - 0.5 * g * dt12 * dt12 - rl1 * rlb * dp12 - (rl1 -
//    rl2) * plb) / dt12;
//  }
//
//  dt12 = all_laser_transforms.last().second.pre_integration->sum_dt_;
//  Vector3d dv12 =
//  all_laser_transforms.last().second.pre_integration->delta_v_;
//
...
bool ImuInitializer::EstimateExtrinsicRotation(
  ...
  for (size_t i = 0; i < window_size; ++i) {
    ...
    Eigen::Quaterniond delta_qij_imu =
        laser_trans_j.second.pre_integration->delta_q_;

    Eigen::Quaterniond delta_qij_laser =
        (laser_trans_i.second.transform.rot.conjugate() *
         laser_trans_j.second.transform.rot)
            .template cast<double>();

    Eigen::Quaterniond delta_qij_laser_from_imu =
        rot_bl.conjugate() * delta_qij_imu * rot_bl;

    double angular_distance =
        180 / M_PI * delta_qij_laser.angularDistance(delta_qij_laser_from_imu);
    ...
    //    cout << (lq_mat * transform_lb.rot.coeffs().template
    //    cast<double>()).transpose() << endl;
    //
    //    cout << (delta_qij_laser * transform_lb.rot.template
    //    cast<double>()).coeffs().transpose() << endl;
    //
    //    cout << (rq_mat * transform_lb.rot.coeffs().template
    //    cast<double>()).transpose() << endl;
    //
    //    cout << (transform_lb.rot.template cast<double>() *
    //    delta_qij_imu).coeffs().transpose() << endl;
  }
...
bool ImuInitializer::Initialization(
    CircularBuffer<PairTimeLaserTransform> &all_laser_transforms,

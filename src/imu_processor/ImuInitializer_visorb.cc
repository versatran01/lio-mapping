...
void RefineGravityAccBias(
    CircularBuffer<PairTimeLaserTransform> &all_laser_transforms,
    CircularBuffer<Vector3d> &Bas, CircularBuffer<Vector3d> &Bgs,
  ...
  if (window_size < 5) {
    LOG(WARNING) << ">>>>>>> window size not enough <<<<<<<";
    return;
  }
  ...
  for (int k = 0; k < 5; ++k) {
    for (size_t i = 0; i < window_size - 1; ++i) {
      ...
      Matrix3d rl2 = laser_trans_j.second.transform.rot.normalized()
                         .template cast<double>()
                         .toRotationMatrix();
      Matrix3d rl3 = laser_trans_k.second.transform.rot.normalized()
                         .template cast<double>()
                         .toRotationMatrix();
      ...
      Matrix3d J_dp12_ba =
          laser_trans_j.second.pre_integration->jacobian_.template block<3, 3>(
              O_P, O_BA);
      Matrix3d J_dp23_ba =
          laser_trans_k.second.pre_integration->jacobian_.template block<3, 3>(
              O_P, O_BA);
      Matrix3d J_dv12_ba =
          laser_trans_j.second.pre_integration->jacobian_.template block<3, 3>(
              O_V, O_BA);
      ...
      tmp_phi = -0.5 * r_WI * SO3::hat(gI_n) * g_norm *
                (dt12 * dt12 * dt23 + dt23 * dt23 * dt12);
      tmp_zeta = rl2 * rlb * J_dp23_ba * dt12 +
                 rl1 * rlb * J_dv12_ba * dt12 * dt23 -
  ...
  //  DLOG(WARNING) << "refined gravity in the inertial frame:  " <<
  //  (R_WI.transpose() * g_refined).transpose();
  DLOG(WARNING) << "acceleration bias initial calibration:  "
                << Bas[0].transpose();
...
void UpdateVelocities(
  ...
  for (size_t i = 0; i < window_size; ++i) {
    ...
    rl1 = laser_trans_i.second.transform.rot.normalized()
              .template cast<double>()
              .toRotationMatrix();
    rl2 = laser_trans_j.second.transform.rot.normalized()
              .template cast<double>()
              .toRotationMatrix();
    rlb = transform_lb.rot.normalized()
              .template cast<double>()
              .toRotationMatrix();

    Vs[i] = (pl2 - pl1 - 0.5 * g * dt12 * dt12 - rl1 * rlb * dp12 -
             (rl1 - rl2) * plb) /
            dt12;
  }

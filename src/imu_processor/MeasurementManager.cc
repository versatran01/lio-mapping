...
void MeasurementManager::SetupRos(ros::NodeHandle &nh) {
  ...
  sub_imu_ = nh_.subscribe<sensor_msgs::Imu>(
      mm_config_.imu_topic, 1000, &MeasurementManager::ImuHandler, this,
      ros::TransportHints().tcpNoDelay());

  sub_compact_data_ = nh_.subscribe<sensor_msgs::PointCloud2>(
      mm_config_.compact_data_topic, 10,
      &MeasurementManager::CompactDataHandler, this);
  //  sub_laser_odom_ =
  //      nh_.subscribe<nav_msgs::Odometry>(mm_config_.laser_odom_topic, 10,
  ...
  while (true) {
    if (mm_config_.enable_imu) {
      ...
      if (imu_buf_.back()->header.stamp.toSec() <=
          compact_data_buf_.front()->header.stamp.toSec() +
              mm_config_.msg_time_delay) {
...
void MeasurementManager::ImuHandler(
    const sensor_msgs::ImuConstPtr &raw_imu_msg) {
...
void MeasurementManager::LaserOdomHandler(
    const nav_msgs::OdometryConstPtr &laser_odom_msg) {
  buf_mutex_.lock();
...
void MeasurementManager::CompactDataHandler(

...
void MarginalizationInfo::Marginalize() {
  ...
  LocalSize(parameter_block_size[reinterpret_cast<long>(it->parameter_blocks[j])]);
              Eigen::MatrixXd jacobian_j = it->jacobians[j].leftCols(size_j);
              if (i == j)
                  tmp_A.block(idx_i, idx_j, size_i, size_j) +=
  jacobian_i.transpose() * jacobian_j; else
              {
                  tmp_A.block(idx_i, idx_j, size_i, size_j) +=
  jacobian_i.transpose() * jacobian_j; tmp_A.block(idx_j, idx_i, size_j, size_i)
  = tmp_A.block(idx_i, idx_j, size_i, size_j).transpose();
              }
          }
          tmp_b.segment(idx_i, size_i) += jacobian_i.transpose() *
  it->residuals;
      }
  }
...
bool MarginalizationFactor::Evaluate(double const *const *parameters,
  ...
  for (int i = 0;
    ...
    if (size != 7)
      ...
      dx.segment<3>(idx + 3) =
          2.0 * (Eigen::Quaterniond(x0(6), x0(3), x0(4), x0(5)).inverse() *

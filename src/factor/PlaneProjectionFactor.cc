...
PlaneProjectionFactor::PlaneProjectionFactor(
    const Eigen::Vector4d &local_coeffi, const Eigen::Vector4d &local_coeffj,
    const double &score)
    : local_coeffi_{local_coeffi}, local_coeffj_{local_coeffj}, score_{score} {}

bool PlaneProjectionFactor::Evaluate(double const *const *parameters,
                                     double *residuals,
  ...
  Eigen::Quaterniond qlb(parameters[2][6], parameters[2][3], parameters[2][4],
                         parameters[2][5]);
  ...
  //  Eigen::Vector3d plane_cen_i{-disi * local_coeffi_.x(), -disi *
  //  local_coeffi_.y(), -disi * local_coeffi_.z()}; Eigen::Vector3d
  //  plane_cen_j{-disj * local_coeffj_.x(), -disj * local_coeffj_.y(), -disj *
  //  local_coeffj_.z()};

  Eigen::Vector4d coeffi_in_j =
      (transform_li.inverse() * transform_lj).transform().matrix().transpose() *
      local_coeffi_;

  if (coeffi_in_j.w() < 0) {
    //    LOG_IF(INFO, coeffi_in_j.w() < 0) << "disi_in_j less than zero: " <<
    //    coeffi_in_j.w();

    coeffi_in_j = (-coeffi_in_j).eval();

    LOG_IF(INFO, coeffi_in_j.w() < 0)
        << "disi_in_j less than zero: " << coeffi_in_j.w();
  }
  ...
  if (jacobians) {
    ...
    if (jacobians[0]) {
      Eigen::Map<Eigen::Matrix<double, 4, 7, Eigen::RowMajor>> jacobian_pose_i(
          jacobians[0]);
      ...
      jaco_i.bottomRightCorner<1, 3>() =
          wi.transpose() * rlb *
          SkewSymmetric(Ri.transpose() *
                        (Pj - Pi - Rj * rlb.transpose() * tlb));
    ...
    if (jacobians[1]) {
      Eigen::Map<Eigen::Matrix<double, 4, 7, Eigen::RowMajor>> jacobian_pose_j(
          jacobians[1]);
      ...
      jaco_j.bottomRightCorner<1, 3>() = wi.transpose() * rlb * Ri.transpose() *
                                         Rj *
    ...
    if (jacobians[2]) {
      Eigen::Map<Eigen::Matrix<double, 4, 7, Eigen::RowMajor>> jacobian_pose_ex(
          jacobians[2]);
      ...
      jaco_ex.topRightCorner<3, 3>() =
          rlb * Rj.transpose() * Ri * SkewSymmetric(rlb.transpose() * wi) -
          rlb * SkewSymmetric(Rj.transpose() * Ri * rlb.transpose() * wi);
      jaco_ex.bottomRightCorner<1, 3>() =
          wi.transpose() *
          (-rlb * Ri.transpose() * (Rj - Ri) *
               SkewSymmetric(rlb.transpose() * tlb) -
           rlb * SkewSymmetric(Ri.transpose() *
                               (Pj - Pi - (Rj - Ri) * rlb.transpose() * tlb)));
...
void PlaneProjectionFactor::Check(double **parameters) {
  ...
  DLOG(INFO) << std::endl
             << Eigen::Map<Eigen::Matrix<double, 4, 7, Eigen::RowMajor>>(
                    jaco[0]);
  DLOG(INFO) << std::endl
             << Eigen::Map<Eigen::Matrix<double, 4, 7, Eigen::RowMajor>>(
                    jaco[1]);
  ...
  residual = info * ((transform_li.inverse() * transform_lj)
                             .transform()
                             .matrix()
                             .transpose() *
                         local_coeffi_ -
  ...
  for (int k = 0; k < 18; k++) {
    ...
    Qj = Eigen::Quaterniond(parameters[1][6], parameters[1][3],
                            parameters[1][4], parameters[1][5]);

    tlb = Eigen::Vector3d{parameters[2][0], parameters[2][1], parameters[2][2]};
    qlb = Eigen::Quaterniond(parameters[2][6], parameters[2][3],
    ...
    tmp_residual = info * ((transform_li.inverse() * transform_lj)
                                   .transform()
                                   .matrix()
                                   .transpose() *
                               local_coeffi_ -

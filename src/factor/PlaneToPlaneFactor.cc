...
PlaneToPlaneFactor::PlaneToPlaneFactor(const Eigen::Vector3d &pi_local,
                                       const Eigen::Vector3d &ni_local,
                                       const Eigen::Vector3d &pj_local,
                                       const Eigen::Vector3d &nj_local)
    : pi_local_{pi_local},
      ni_local_{ni_local},
      pj_local_{pj_local},
      nj_local_{nj_local} {
...
bool PlaneToPlaneFactor::Evaluate(double const *const *parameters,
  ...
  //  eps_).select(saes2.eigenvalues().array(), 0)); Eigen::VectorXd S_inv =
  //  Eigen::VectorXd((saes2.eigenvalues().array() >
  //  eps_).select(saes2.eigenvalues().array().inverse(), 0));
  //
  ...
  Eigen::Matrix3d sqrt_info =
      Eigen::LLT<Eigen::Matrix3d>(M).matrixL().transpose();
  //  sqrt_info *= (500);
  ...
  if (jacobians) {
    ...
    if (jacobians[0]) {
      ...
      jaco_i.rightCols<3>() =
          rlb * SkewSymmetric(Ri.transpose() * Rj * rlb.transpose() * p_a) +
    ...
    if (jacobians[1]) {
      ...
      jaco_j.rightCols<3>() =
          -rlb * Ri.transpose() * Rj * SkewSymmetric(rlb.transpose() * p_a) +
    ...
    if (jacobians[2]) {
      Eigen::Map<Eigen::Matrix<double, 3, 7, Eigen::RowMajor>> jacobian_pose_ex(
          jacobians[2]);
      Eigen::Matrix<double, 3, 6> jaco_ex;

      jaco_ex.setZero();

      jaco_ex.leftCols<3>() = -rlb * Ri.transpose() * Rj * rlb.transpose() +
                              Eigen::Matrix3d::Identity();
      Eigen::Vector3d tmp_t = p_a - tlb;
      jaco_ex.rightCols<3>() =
          rlb * Ri.transpose() * Rj * SkewSymmetric(rlb.transpose() * tmp_t) -
          rlb * SkewSymmetric(Ri.transpose() * Rj * rlb.transpose() * tmp_t) -
          rlb * SkewSymmetric(Ri.transpose() * (Pj - Pi - tlb));
...
void PlaneToPlaneFactor::Check(double **parameters) {
  ...
  for (int k = 0; k < 18; k++) {
    ...
    tmp_residual =
        sqrt_info * (transform_b_a.rot * p_a + transform_b_a.pos - p_b);

...
PivotPointPlaneFactor::PivotPointPlaneFactor(const Eigen::Vector3d &point,
    ...
    : point_{point}, coeff_{coeff} {
  /// add new point and coeff
}

bool PivotPointPlaneFactor::Evaluate(double const *const *parameters,
                                     double *residuals,
                                     double **jacobians) const {
  TicToc tic_toc;

  Eigen::Vector3d P_pivot(parameters[0][0], parameters[0][1], parameters[0][2]);
  Eigen::Quaterniond Q_pivot(parameters[0][6], parameters[0][3],
                             parameters[0][4], parameters[0][5]);

  Eigen::Vector3d Pi(parameters[1][0], parameters[1][1], parameters[1][2]);
  Eigen::Quaterniond Qi(parameters[1][6], parameters[1][3], parameters[1][4],
                        parameters[1][5]);
  ...
  if (jacobians) {
    ...
    if (jacobians[1]) {
      ...
      jaco_i.rightCols<3>() = w.transpose() * rlb * Rp.transpose() * Ri *
    ...
    if (jacobians[2]) {
      ...
      right_info_mat = Qlpivot.conjugate().normalized() * right_info_mat *
                       Qlpivot.normalized();

      Eigen::Matrix<double, 1, 6> jaco_ex;
      //  NOTE: planar extrinsic
      //       jaco_ex.leftCols<3>() = w.transpose() * (I3x3 - rlb *
      //       Rp.transpose() * Ri * rlb.transpose()) * right_info_mat;
      jaco_ex.leftCols<3>() =
          w.transpose() * (I3x3 - rlb * Rp.transpose() * Ri * rlb.transpose());
      jaco_ex.rightCols<3>() =
          w.transpose() * rlb *
          (-SkewSymmetric(Rp.transpose() * Ri * rlb.transpose() *
                          (point_ - tlb)) +
           Rp.transpose() * Ri *
               SkewSymmetric(rlb.transpose() * (point_ - tlb)) -
           SkewSymmetric(Rp.transpose() * (Pi - P_pivot)));
...
void PivotPointPlaneFactor::Check(double **parameters) {
  ...
  //  for (int k = 0; k < 6; k++) {
  for (int k = 0; k < 18; k++) {
    Eigen::Vector3d P_pivot(parameters[0][0], parameters[0][1],
                            parameters[0][2]);
    Eigen::Quaterniond Q_pivot(parameters[0][6], parameters[0][3],
                               parameters[0][4], parameters[0][5]);
    ...
    double tmp_residual =
        (coeffw.transpose() * (Qlpi * point_ + Plpi) + coeffb);

...
void PointCloudHandler(const sensor_msgs::PointCloud2ConstPtr &raw_points_msg) {
  ...
  //  pcl::PointCloud<pcl::PointXYZI>::ConstPtr laser_cloud_in_ptr(new
  //  pcl::PointCloud<pcl::PointXYZI>(laser_cloud_in));
  pcl::PointCloud<pcl::PointXYZI>::Ptr transformed_ptr(
      new pcl::PointCloud<pcl::PointXYZI>());
  pcl::PointCloud<pcl::PointXYZI> laser_cloud_out;

  pcl::transformPointCloud(laser_cloud_in, *transformed_ptr,
                           transform_to_world);
  ...
  pcl::transformPointCloud(tmp_cloud, laser_cloud_out,
                           transform_to_world.inverse());

  PublishCloudMsg(pub_filtered_cloud, laser_cloud_out,
                  raw_points_msg->header.stamp,
                  raw_points_msg->header.frame_id);

  br_ptr->sendTransform(
      tf::StampedTransform(tf_transform, raw_points_msg->header.stamp,
                           "imu_link_kaist", "right_velodyne"));
}
...
int main(int argc, char **argv) {
  ...
  pub_filtered_cloud =
      nh.advertise<sensor_msgs::PointCloud2>("/filtered_points", 2);

  Eigen::Matrix3f R_inv;
  R_inv << -4.91913910e-01, -5.01145813e-01, -7.11950546e-01, 7.13989130e-01,
      -7.00156621e-01, -4.78439170e-04, -4.98237120e-01, -5.08560301e-01,
      7.02229444e-01;

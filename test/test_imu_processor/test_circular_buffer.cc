...
TEST(CirclurBufferTest, ForLoopTest) {
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    pre_integrations.push(std::make_shared<IntegrationBase>(
        IntegrationBase(acc_last_, gyr_last_, Bas_, Bgs_)));
  }
...
TEST(CirclurBufferTest, SharedPtrTest) {
  //  CircularBuffer<shared_ptr<IntegrationBase> > pre_integrations =
  //  CircularBuffer<shared_ptr<IntegrationBase> >(5 + 1);
  CircularBuffer<shared_ptr<IntegrationBase> > pre_integrations{5 + 1};
  ...
  DLOG(INFO)
      << pre_integrations[pre_integrations.size() - 1]->acc0_.transpose();

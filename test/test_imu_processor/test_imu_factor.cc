...
DEFINE_string(filename, "/home/hyye/dev_ws/src/lio/test/data/imu_pose_vel.txt",
              "filename to load");
DEFINE_bool(use_opt, false, "whether to use opt");
DEFINE_bool(use_marg, false, "marginalization");
// DEFINE_string(filename,
// "/home/hyye/dev_ws/src/lio/test/data/imu_pose_vel_noise.txt", "filename to
// load");
...
void Double2Vector() {
  ...
  Vector3d origin_R00 = R2ypr(Quaterniond(para_pose[0][6], para_pose[0][3],
  ...
  for (int i = 0; i <= WINDOW_SIZE; i++) {
    ...
    Vs[i] = rot_diff * Vector3d(para_speed_bias[i][0], para_speed_bias[i][1],
                                para_speed_bias[i][2]);
    ...
    Bas[i] = Vector3d(para_speed_bias[i][3], para_speed_bias[i][4],
                      para_speed_bias[i][5]);

    Bgs[i] = Vector3d(para_speed_bias[i][6], para_speed_bias[i][7],
                      para_speed_bias[i][8]);
  }
}
...
TEST(ImuFactorTest, pimtest) {
  ...
  save_points << "t qw0 qx0 qy0 qz0 x0 y0 z0 vx0 vy0 vz0 qw1 qx1 qy1 qz1 x1 y1 "
                 "z1 vx1 vy1 vz1"
              << std::endl;
  ...
  shared_ptr<IntegrationBase> tmp_pre_integrations =
      std::make_shared<IntegrationBase>(
          IntegrationBase(acc_last, gyr_last, tmp_Bas, tmp_Bgs, pim_config));
  ...
  for (int data_idx = 0; data_idx < imu_data.size(); ++data_idx) {
    ...
    if (data_idx % INTERVAL == 0) {
      ...
      tmp_pre_integrations = std::make_shared<IntegrationBase>(
          IntegrationBase(acc_last, gyr_last, Bas[cir_buf_count],
                          Bgs[cir_buf_count], pim_config));
    ...
    if (cir_buf_count < WINDOW_SIZE) {
      ...
      //          DLOG(INFO) << "pre: " << (pre_integrations[j]->Evaluate(Ps[i],
      //          Qs[i], Vs[i], Bas[i], Bgs[i],
      //                                                               Ps[j],
      //                                                               Qs[j],
      //                                                               Vs[j],
      //                                                               Bas[j],
      ...
      //                                                               g_vec)).transpose();
      //          DLOG(INFO) << "pre_gt: " <<
      //          (pre_integrations[j]->Evaluate(Ps_gt[i], Qs_gt[i], Vs_gt[i],
      //          Bas_gt[i], Bgs_gt[i],
      //                                                                  Ps_gt[j],
      //                                                                  Qs_gt[j],
      //                                                                  Vs_gt[j],
      ...
      //                                                                  g_vec)).transpose();
      //
      //          DLOG(INFO) << Eigen::Map<Eigen::Matrix<double, 15,
      //          1>>(res).transpose(); DLOG(INFO) << "jaco0:" << std::endl <<
      //          Eigen::Map<Eigen::Matrix<double, 15, 7,
      //          Eigen::RowMajor>>(jaco[0]); DLOG(INFO) << "jaco1:" <<
      //          std::endl << Eigen::Map<Eigen::Matrix<double, 15, 9,
      //          Eigen::RowMajor>>(jaco[1]); DLOG(INFO) << "jaco2:" <<
      //          std::endl << Eigen::Map<Eigen::Matrix<double, 15, 7,
      //          Eigen::RowMajor>>(jaco[2]); DLOG(INFO) << "jaco3:" <<
      //          std::endl << Eigen::Map<Eigen::Matrix<double, 15, 9,
      //          Eigen::RowMajor>>(jaco[3]);
      //
      ////          DLOG(INFO) << "covariance: " << endl <<
      ///pre_integrations[j]->covariance_;
      //
...

      {
        ...
        vector<ceres::internal::ResidualBlock *> res_ids_pim, res_ids_pim_gt,
            res_marg;
        ...
        for (int i = 0; i < WINDOW_SIZE; ++i) {
          ...
          ceres::internal::ResidualBlock *res_id = problem.AddResidualBlock(
              f, NULL, para_pose[i], para_speed_bias[i], para_pose[j],
              para_speed_bias[j]);

          res_ids_pim.push_back(res_id);

          DLOG(INFO) << "pre" << i << ": "
                     << (pre_integrations[j]->Evaluate(
                             Ps[i], Eigen::Quaterniond(Rs[i]), Vs[i], Bas[i],
                             Bgs[i], Ps[j], Eigen::Quaterniond(Rs[j]), Vs[j],
                             Bas[j], Bgs[j]))
                            .squaredNorm();

          DLOG(INFO) << "pre_gt" << i << ": "
                     << (pre_integrations[j]->Evaluate(
                             Ps_gt[i], Qs_gt[i], Vs_gt[i], Bas_gt[i], Bgs_gt[i],
                             Ps_gt[j], Qs_gt[j], Vs_gt[j], Bas_gt[j],
                             Bgs_gt[j]))
                            .squaredNorm();
        ...
        for (int i = 0; i < WINDOW_SIZE + 1; ++i) {
          ceres::LocalParameterization *local_parameterization =
              new PoseLocalParameterization();
          problem_gt.AddParameterBlock(para_pose_gt[i], SIZE_POSE,
                                       local_parameterization);
        ...
        for (int i = 0; i < WINDOW_SIZE; ++i) {
          int j = i + 1;
          ImuFactor *f = new ImuFactor(pre_integrations[j]);
          ceres::internal::ResidualBlock *res_id = problem_gt.AddResidualBlock(
              f, NULL, para_pose_gt[i], para_speed_bias_gt[i], para_pose_gt[j],
        ...
        // region residual before optimization
        {
          ...
          DLOG(INFO) << para_speed_bias[1][3] << " " << para_speed_bias[1][4]
                     << " " << para_speed_bias[1][5];
          //          DLOG(INFO) << para_pose[0][0] << " " << para_pose[0][1] <<
          //          " " << para_pose[0][2];
        ...
        // region Marginalization
        {
          if (FLAGS_use_marg) {
            ...
            if (pre_integrations[1]->sum_dt_ < 10.0) {
              ...
              ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(
                  imu_factor, NULL,
                  vector<double *>{para_pose[0], para_speed_bias[0],
                                   para_pose[1], para_speed_bias[1]},
                  vector<int>{0, 1});
              marginalization_info->AddResidualBlockInfo(residual_block_info);
            }
            ...
            for (int i = 1; i < WINDOW_SIZE + 1; ++i) {
              addr_shift[reinterpret_cast<long>(para_pose[i])] =
                  para_pose[i - 1];
              addr_shift[reinterpret_cast<long>(para_speed_bias[i])] =
                  para_speed_bias[i - 1];
            }
      ...
      //      DLOG(INFO) << "data.imu_velocity: " <<
      //      data.imu_velocity.transpose(); DLOG(INFO) << "tmp_Vs: " <<
      //      tmp_Vs.transpose();
      DLOG(INFO) << "======= TEST =======";
      ...
      save_points << curr_time << " " << tmp_q_gt.w() << " " << tmp_q_gt.x()
                  << " " << tmp_q_gt.y() << " " << tmp_q_gt.z() << " "
                  << tmp_p_gt(0) << " " << tmp_p_gt(1) << " " << tmp_p_gt(2)
                  << " " << tmp_v_gt(0) << " " << tmp_v_gt(1) << " "
                  << tmp_v_gt(2) << " " << tmp_q.w() << " " << tmp_q.x() << " "
                  << tmp_q.y() << " " << tmp_q.z() << " " << tmp_Ps(0) << " "
                  << tmp_Ps(1) << " " << tmp_Ps(2) << " " << tmp_Vs(0) << " "
                  << tmp_Vs(1) << " " << tmp_Vs(2) << " " << std::endl;
